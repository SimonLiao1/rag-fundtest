...
【零、题干角色与主体识别（先做）】
- 请先从题目中识别并明确涉及的角色或主体（如：基金管理人、基金经理、托管人、投资者、关联人、亲属等）。
- 将题干中的角色与教材中对应的职责主体进行匹配，后续判断必须基于匹配后的主体进行。
- 若题干中的角色无法与教材主体形成合理对应，请在置信度中体现不确定性。
...
- 在做出最终选择前，请进行一次证据覆盖度校验：
  - 若当前判断仅基于单一语义来源（如单一 Parent 或单一条款），
    请降低置信度，并谨慎检查是否存在反例或未覆盖条件。
...
Confidence: [0.0 到 1.0 之间的数字，证据越明确、覆盖度越高，置信度越高]
Evidence:
